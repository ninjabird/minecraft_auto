...
# Set path variable
USERPATH="/home/ec2-user/.local/bin:/home/ec2-user/bin:/usr/local/bin:/usr/bin:/usr/local/sbin:/usr/sbin"
PathLength=${#USERPATH}
...
Permissions=$(sudo bash /home/ec2-user/minecraftbe/minecraft/fixpermissions.sh -a)
...
Rotate=$(
    pushd /home/ec2-user/minecraftbe/minecraft/backups
...
if [ "$?" != 0 ]; then
    ...
    if [[ "$PinFile" == *"zip" ]] && [[ "$InstalledFile" == "$PinFile" ]]; then
        ...
        if [ ! -z "$DownloadFile" ]; then
            if [ ! -e /home/ec2-user/minecraftbe/minecraft/server.properties ]; then
            ...
            Permissions=$(chmod u+x /home/ec2-user/minecraftbe/minecraft/bedrock_server >/dev/null)
...
if [ ! -e /home/ec2-user/minecraftbe/minecraft/allowlist.json ]; then
    echo "Creating default allowlist.json..."
    echo '[]' >/home/ec2-user/minecraftbe/minecraft/allowlist.json
fi
if [ ! -e /home/ec2-user/minecraftbe/minecraft/permissions.json ]; then
    echo "Creating default permissions.json..."
    echo '[]' >/home/ec2-user/minecraftbe/minecraft/permissions.json
fi
ContentLogging=$(grep "content-log-file-enabled" /home/ec2-user/minecraftbe/minecraft/server.properties)
if [ -z "$ContentLogging" ]; then
    echo "" >> /home/ec2-user/minecraftbe/minecraft/server.properties
    echo "content-log-file-enabled=true" >> /home/ec2-user/minecraftbe/minecraft/server.properties
    echo "# Enables logging content errors to a file" >> /home/ec2-user/minecraftbe/minecraft/server.properties
fi
...
if [[ "$CPUArch" == *"aarch64"* ]]; then
    ...
    if [ -n "$(which box64)" ]; then
        ...
        BASH_CMD="LD_LIBRARY_PATH=/home/ec2-user/minecraftbe/minecraft /home/ec2-user/minecraftbe/minecraft/bedrock_server"
    fi
else
    BASH_CMD="LD_LIBRARY_PATH=/home/ec2-user/minecraftbe/minecraft /home/ec2-user/minecraftbe/minecraft/bedrock_server"
fi
...
screen -L -Logfile logs/minecraft.log -dmS minecraft /bin/bash -c "${BASH_CMD}"
